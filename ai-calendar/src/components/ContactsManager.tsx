...
export default function ContactsManager({ walletAddress }: ContactsManagerProps) {
  // State
  ...
  const loadStoredContacts = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/calendar/google/sync-contacts?wallet_address=${walletAddress}`
      );
      const data = await response.json();
  ...
  }, [walletAddress, checkConnection, loadStoredContacts]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold mb-2 text-white">Contact Management</h2>
      ...
      {connectionStatus === 'not_connected' && (
        <div className="bg-yellow-900/20 border border-yellow-600 rounded-lg p-4">
          <p className="text-yellow-400">
            ⚠️ Google Calendar not connected. Please connect in the Settings tab first.
          </p>
        </div>
      )}
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-xl font-semibold text-white">Gmail Contact Sync</h3>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              disabled={!walletAddress || syncStatus !== 'idle' || connectionStatus === 'not_connected'}
      ...
      {storedContacts.length === 0 && syncStatus === 'idle' && (
        <div className="bg-gray-800 rounded-lg p-12 text-center">
          ...
          <p className="text-gray-500 text-sm">
            Click one of the sync buttons above to extract email addresses from your Gmail
          </p>
        </div>
      )}
    </div>
